...
const Header = () => {
  ...
  const handleSearchChange = async (e) => {
    ...
    if (query.length > 3) {
      // Consider changing the length based on your requirement
      try {
...
//         setSuggestions(response.data.data); // Adjust according to response format
//       } catch (error) {
//         enqueueSnackbar('Failed to fetch suggestions.', { variant: 'error' });
//       }
//     } else {
...
//     try {
//       const response = await searchProduct(search);
//       // Navigate to the search results page with relevant query type and term
//       if (response.data.data.length === 1) {
//         navigate(`/product/details/${response.data.data[0]._id}`); // Assuming single result means exact product
//       } else {
//         navigate(`/product/search/${search}`); // Adjust path and logic as needed
//       }
//     } catch (error) {
//       enqueueSnackbar(`Error fetching products: ${error.message}`, { variant: 'error' });
//     }
